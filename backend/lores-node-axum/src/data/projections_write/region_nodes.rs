...
use crate::data::entities::RegionNode;
...
impl RegionNodesWriteRepo {
    ...
    pub async fn upsert(&self, pool: &SqlitePool, node: &RegionNode) -> Result<(), sqlx::Error> {
        sqlx::query!(
            "INSERT INTO region_nodes (node_id, region_id, name, public_ipv4, domain_on_local_network, domain_on_internet)
            VALUES (?, ?, ?, ?, ?, ?)
            ON CONFLICT(node_id, region_id) DO UPDATE SET name = excluded.name, public_ipv4 = excluded.public_ipv4, domain_on_local_network = excluded.domain_on_local_network, domain_on_internet = excluded.domain_on_internet",
            node.node_id,
            node.region_id,
            node.name,
            node.public_ipv4,
