...
pub async fn truncate_all(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    ...
    let tables: Vec<String> = sqlx::query("SELECT name FROM sqlite_master WHERE type = 'table' AND name != '_sqlx_migrations'")
        .fetch_all(&mut *conn)
        .await?
        .into_iter()
    ...
    for table in tables {
        sqlx::query(&format!("DELETE FROM \"{table}\"")).execute(&mut *conn).await?;
    }
