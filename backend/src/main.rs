...
async fn main() {
    ...
    // DATABASE
    let projection_db = db::prepare_database("projection")
        .await
    ...
    start_panda(&projection_db, &container).await;
    start_panda_event_handler(channel_rx, projection_db.clone());
...
async fn start_panda(projection_db: &SqlitePool, container: &P2PandaContainer) {
    let repo = ThisP2PandaNodeRepo::init();

    match repo.get_network_name(projection_db).await {
        Ok(network_name) => {
    ...
    match repo.get_or_create_private_key(projection_db).await {
        Ok(private_key) => {
    ...
    let bootstrap_details = repo.get_bootstrap_details(projection_db).await.unwrap();
...
fn start_panda_event_handler(channel_rx: mpsc::Receiver<LoResEvent>, projection_db: SqlitePool) {
    tokio::spawn(async move {
        let mut events_rx = channel_rx;

        // Start the event loop to handle events
        while let Some(event) = events_rx.recv().await {
            handle_event(event, &projection_db).await;
        }
    });
}
