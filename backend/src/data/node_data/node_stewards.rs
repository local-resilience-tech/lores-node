...
impl NodeStewardsRepo {
    ...
    pub async fn all(&self, pool: &SqlitePool) -> Result<Vec<NodeStewardRow>, sqlx::Error> {
        let nodes = sqlx::query_as::<Sqlite, NodeStewardRow>(
            "
            SELECT id, name, hashed_password, password_reset_token, password_reset_token_expires_at, active
            FROM node_stewards
            ",
        )
        .fetch_all(pool)
    ...
    pub async fn create(&self, pool: &SqlitePool, row: &NodeStewardRow) -> Result<(), sqlx::Error> {
        sqlx::query::<Sqlite>(
            "
            INSERT INTO node_stewards (id, name, hashed_password, password_reset_token, password_reset_token_expires_at, active)
            VALUES (?, ?, ?, ?, ?, ?)
            ",
        )
        .bind(&row.id)
